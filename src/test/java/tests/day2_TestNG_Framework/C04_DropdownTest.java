...
public class C04_DropdownTest {
    ...
    public void dropdownTest(){
        ...
        Select selectGun = new Select(testotomasyonuFormPage.gunDropdown);
        selectGun.selectByIndex(5);

        // 3- Dogum tarihi ay seçeneğinden value kullanarak Nisan’i secin
        Select selectAy = new Select(testotomasyonuFormPage.ayDropdown);
        selectAy.selectByValue("nisan");

        // 4- Dogum tarihi yil seçeneğinden visible text kullanarak 1990’i secin
        Select selectYil = new Select(testotomasyonuFormPage.yilDropdown);
        ...
        System.out.println(ReusableMethods.convertStringList(selectAy.getOptions()));

        // 7- Ay Dropdown menusunun boyutunun 13 olduğunu test edin
        int expectedMenuSize = 13;
        int actualMenuSize = selectAy.getOptions().size();
